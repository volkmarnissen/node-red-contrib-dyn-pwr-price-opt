...
export class BaseNode<Config> {
  ...
  private setField(target: Object, field: IfieldDescription, value: any, type: string) {
      ...
      switch (type) {
        case "num":
          ...
          if( field.required && Number.isNaN(value))
            throw new Error("Required field " + field.name + " is not a valid number ")
          break;
        case "bool":
          newValue = "true " == value;
          break;
    ...
    if (target[field.name] == undefined)
      Object.defineProperty(target, field.name, { value: newValue, writable: true });
    else target[field.name] = newValue;
  }
  private findMissingRequiredFields( fields:IfieldDescription[], config:any){
    let rc =  fields.find(bf=>bf.required && ! config.hasOwnProperty(bf.name) )
    if( rc != null){
  ...
  private overwriteConfigProperties(target: Config, config: any): void {
    ...
    for (let field of Object.keys(config)) {
      if (config.hasOwnProperty(field)){
        ...
        this.setField(target, {name:field, required:cType!= null? cType.required:false} , config[field], fieldType);
      ...
      for (let typeFieldDescription of this.configType.typeFields) {
        ...
        if (! typeField.endsWith("Type") && config[typeField] != undefined ) {
          ...
          if (typeof config[typeField] != "string")
            this.setField(target, typeFieldDescription, config[typeField], "copy");
          else
            if (config[typeField] == "")
              this.setField(target, { name:typeField, required:false }, undefined, "copy");
            else
              if( ["num", "bool", "json"].includes(String(config[typeField + "Type"])) )
                ...
                this.setField(target, { name:typeField, required:false }, config[typeField], "copy");
        }
      }
    ...
    if (this.configType.numberFields)
      for (let numberFieldDescription of this.configType.numberFields)
        if (config[numberFieldDescription.name] != undefined)
          this.setField(target, numberFieldDescription, config[numberFieldDescription.name], "num");
    if (this.configType.booleanFields)
