...
export  abstract class BaseNodeEnergyConsumption<T> extends BaseNode<T> {
   ...
   constructor(config: any, tConfig:TypeDescription, RED:any) {
    ...
    protected getEstimconsumptionperiods():number{ return 0}
    ...
    onTime(time: number): void {
      ...
      if( this.getNightTimeStartHour()!= undefined && 
        this.getNightTimeEndHour() != undefined && this.getNightTimeOutput() != undefined
        && (currentHour < this.getNightTimeEndHour() || currentHour > this.getNightTimeStartHour()))
        value = this.getNightTimeOutput()
      else{
