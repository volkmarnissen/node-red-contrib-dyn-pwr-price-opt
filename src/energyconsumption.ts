...
export class EnergyConsumption{
    ...
    private getStartTime(time:number):number{
        ...
        let periodMinutes = Math.round(Math.floor(startPeriodTime.getMinutes() / 60 / this.config.priceInfo.prices.periodlength ) * (60 * this.config.priceInfo.prices.periodlength))
    ...
    checkConfig(){
      if( !this.config )
        throw new Error( "EnergyConsumption: No config")
    ...
    private buildSchedule( currentTime: number) {
      ...
      range.forEach((entry) => {
        ...
        let rangeSize =
          Math.floor((priceRangeSize - cheapestHours - mostExpHours) /
