...
export class HeatingNode extends BaseNodeEnergyConsumption<HeatingConfig> {
 ...
 constructor(config: any, RED:any) {
    ...
    this.registerInputListener(/^payload|currenttemperature$/g, this.readHeatpumpPayload.bind(this))
  }
  ...
  getNightTimeEndHour(): number {
   ...
   getEstimconsumptionperiods():number{
    ...
    if( this.currenttemperature == undefined || this.currenttemperature >=this.config.minimaltemperature)
      return 0

    return Math.floor((this.config.minimaltemperature - currenttemperature) / decreasetemperatureperhour * this.getPeriodLength()  ) 
  }
  getPeriodLength(){
    let periodlength = 1
  ...
  getEstimstoreperiods():number{
    ...
    if( currenttemperature != undefined && this.config.maximaltemperature){
      if( currenttemperature > this.config.maximaltemperature)
        return 0// no heating required
      increasetemperatureperhour *= (this.config.maximaltemperature - currenttemperature) / 
        (this.config.maximaltemperature  - this.config.minimaltemperature) 
    }

    return Math.floor((this.config.maximaltemperature - currenttemperature) / this.config.increasetemperatureperhour * this.getPeriodLength())    
    ;
